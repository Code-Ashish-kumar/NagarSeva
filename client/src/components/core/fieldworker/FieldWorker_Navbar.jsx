...
import { links } from '../../../data/fieldworker_navbar';
...
const LINK_ICONS = {
  ...
  'Reports': <FiCheckCircle className="w-4.5 h-4.5" />,
};

const FieldWorker_Navbar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  // Load designation details dynamically
          ...
          <>
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <span className="text-xs font-bold text-white block truncate leading-tight">{profile?.name || user.name}</span>
                <span className="text-[10px] text-white/50 block truncate leading-none mt-0.5">{profile?.designation || 'Field Worker'}</span>
              </div>
              <svg
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 bottom-full mb-3 w-64 bg-white border border-gray-150 rounded-sm shadow-xl py-3.5 z-50 text-gray-800 animate-in fade-in slide-in-from-bottom-2 duration-150">
                <div className="px-4 py-3 flex flex-col items-center border-b border-gray-100">
                  ...
                  <span className="text-sm font-extrabold text-gray-900 leading-snug">{profile?.name || user.name}</span>
...
export default FieldWorker_Navbar;
